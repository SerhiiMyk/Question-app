...
export class LStorageService {
  ...
  public addStorageKey(storageKey: any, defaultValue: any): void {
    if (!this._storage.hasOwnProperty(storageKey)) {
      let storageData: any = window.localStorage.getItem(storageKey);
      this._storage[storageKey] = (storageData) ? JSON.parse(storageData) : defaultValue;
    }
  }
...
export function LocalStorage(autoSave: boolean, key ?: string, defaultValue?:any) {
  return function (target: Object, propName: string) {
    let propNameId: string = (key) ? key : propName;

    storageService.addStorageKey(propNameId, defaultValue);
